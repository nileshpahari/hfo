...
// Dynamically detect Flask backend URL (tries 127.0.0.1 then localhost)
let FLASK_BACKEND_URL = null;

async function getBackendBase() {
    if (FLASK_BACKEND_URL) return FLASK_BACKEND_URL;
    const cached = localStorage.getItem('backendBaseUrl');
    if (cached) {
        FLASK_BACKEND_URL = cached;
        return FLASK_BACKEND_URL;
    }
    const candidates = [
        'http://127.0.0.1:5000',
        'http://localhost:5000'
    ...
    // Fallback (may still fail until backend is up)
    FLASK_BACKEND_URL = candidates[1];
    return FLASK_BACKEND_URL;
}
...
function uploadAudio(audioBlob) {
    ...
    formData.append("slackEnabled", slackEnabledCheckbox ? slackEnabledCheckbox.checked : false);
    formData.append("notionEnabled", notionEnabledCheckbox ? notionEnabledCheckbox.checked : false);
